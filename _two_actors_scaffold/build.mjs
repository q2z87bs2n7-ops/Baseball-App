...
// sw.js CACHE and index.html cache-bust query strings are mutated by regex
// replacement after every build. esbuild's `define` only replaces identifiers
// on the first build, so subsequent builds would leave them stale.
function rewriteVersionedFiles() {
  ...
  const nextSw = sw.replace(/const CACHE\s*=\s*["']alg-v[^"']+["']\s*;/, `const CACHE = "alg-v${VERSION}";`);
  if (nextSw !== sw) {
    writeFileSync(swPath, nextSw);
    console.log(`esbuild: rewrote sw.js CACHE to alg-v${VERSION}`);
  }
}
