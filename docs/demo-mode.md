...
| `selectFocusGame` | tension-scores live games; respects `state.focusIsManual` (manual override blocks auto-switch) | early-returns when `state.focusIsManual` is true (demo viewer manually picked a game); otherwise walks `focusTrack[]` for `ts ≤ demoCurrentTime`, falls back to `focusTrack[0]` (recorder user's focus target) until demo crosses the recording window, falls back to tension scoring if `focusTrack` empty. Does **not** assign `state.focusIsManual` from the captured `entry.isManual` flag — the captured flag is recording metadata, the viewer's manual flag is runtime state, mixing them caused manual overrides to revert unexpectedly. |
