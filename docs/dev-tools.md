...
> **Where the code lives:** `toggleDevTools`, `updateTuning`, `resetTuning`, the panel-wide click delegator, color-override + theme-lock helpers, and `confirmDevToolsChanges` are in `src/dev/tuning.js`. All inspector subsystems (Log Capture, App State, Network Trace, localStorage, Service Worker, Test Notification, Live Controls, Diagnostic Snapshot) are in `src/dev/panels.js`. The QA test panels (YouTube Debug, Video Debug, News Source Test) are in `src/dev/{youtube,video,news}-debug.js`. Where this doc says "in `app.js`" below, read it as "in the corresponding `src/dev/*.js` module" — the legacy `app.js` (USE_BUNDLE=false fallback) inlines the same functions for backwards compatibility.
