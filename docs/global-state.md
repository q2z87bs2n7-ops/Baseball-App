...
let stolenBaseEvents = []               // live stolen base plays + caught stealing (caught:true flag) for carousel story generator. Populated by pollGamePlays from playEvents[]; keyed by playId UUID. Pruned >30 min in buildStoryPool. (not added to feed)
let actionEvents     = []               // live mid-AB action events: pickoff outs, pitching changes, pinch hitters/runners, replay reviews. Populated by pollGamePlays from playEvents[]; keyed by playId UUID. Pruned >30 min in buildStoryPool. (not added to feed)
let seenActionEventIds=new Set()        // playId UUIDs already processed by pollGamePlays — dedup across polls. Reset on date rollover.
