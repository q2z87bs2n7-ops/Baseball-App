...
- Version number at bottom (`v2.57.2`)
...
- **⚡ Pulse Tuning** (`<details>` collapsible) — live-edit inputs for `devTuning`:
  ...
  - Reset to Defaults button restores all to `devTuningDefaults`
- **📖 Carousel Story Rules** (`<details>` collapsible) — per-story tuning knobs wired to `devTuning`:
  ...
  - *Bases Loaded* — Enable checkbox (`basesloaded_enable`; default true) + Priority (`basesloaded_priority`; default 88)
- **🎨 Theme Tuning** (`<details>` collapsible) — color pickers for App Theme (Primary, Secondary, Dark BG, Card BG) and Pulse Theme (Dark BG, Card BG); Copy button captures current live CSS vars into pickers; Lock Theme checkbox (`devColorLocked`) disables auto-switching when team changes
...
| `toggleDevTools()` | Shows/hides `#devToolsPanel`; on open, populates all tuning inputs (Pulse Tuning + Carousel Story Rules + Theme Tuning) from live `devTuning`/`devColorLocked` values |
