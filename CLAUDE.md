...
- `#pulseTopBar` — brand strip above `#gameTicker`; shares CSS rule with `#ydSectionBar` (Yesterday Recap heading) so both render identically. Contains: ⚡ MLB PULSE wordmark + tagline (`.ptb-kicker` / `.ptb-brand` / `.ptb-tagline`) on the left; right side wraps `#ptbYestBtn` ("YESTERDAY'S RECAP" pill — `display:none` until `loadYesterdayCache()` resolves with data, also shown in demo mode) + `#myTeamLensBtn` (MY TEAM toggle — **only visible when live feed is active**; added to `hideWhenEmpty` array in `updateFeedEmpty()` so it hides during hype/post-slate/intermission states).
...
| `updatePulseToggle()` | Updates `#pulseSchemeToggle` background and `#pulseSchemeKnob` position to reflect current `pulseColorScheme`. |
