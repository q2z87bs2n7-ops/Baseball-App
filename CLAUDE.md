...
Everything — HTML, CSS, JavaScript — is in `index.html`. No imports, no modules, no external scripts for the app itself (except `focusCard.js` and `pulse-card-templates.js` loaded via `<script src>`). Edit the file, push to branch, done.
