...
Everything — HTML, CSS, JavaScript — is in `index.html`. No imports, no modules, no external scripts for the app itself (except `pulse-card-templates.js` loaded via `<script src>`). Edit the file, push to branch, done.
...
- **Confirm Changes** — sticky footer button; reads all numeric inputs from Pulse Tuning + Carousel Story Rules and calls `updateTuning()` for each; flashes green "✓ Applied!" for 1.5s. Binary toggles (checkboxes, on/off knobs, color pickers) apply immediately without needing Confirm.
...
| `isDesktop()` | Returns true when viewport ≥1025px AND `!('ontouchstart' in window)`. Used to hide 🔔 push row on desktop. **Do not use `navigator.maxTouchPoints`** — Chrome on Mac reports 1 via Apple trackpad, causing false negatives. |
| `updatePushRowVisibility()` | Shows/hides `#pushRow` based on `isDesktop() && !devShowPushOnDesktop`; called on page load and from `togglePushOnDesktop()` |
| `togglePushOnDesktop()` | Flips `devShowPushOnDesktop`, persists to localStorage, updates Dev Tools toggle knob, calls `updatePushRowVisibility()` |
| `confirmDevToolsChanges()` | Reads all numeric Dev Tools inputs (Pulse Tuning + Carousel Story Rules) and calls `updateTuning()` for each; flashes "✓ Applied!" green on the button for 1.5s |
| `toggleDevTools()` | Shows/hides `#devToolsPanel`; on open, populates all tuning inputs (Pulse Tuning + Carousel Story Rules + Theme Tuning) from live `devTuning`/`devColorLocked`/`devShowPushOnDesktop` values. Uses `p.style.display !== 'block'` (not `=== 'none'`) to detect closed state — panel starts with CSS display:none (no inline style), so checking for 'none' would fail on first open. |
| `updateTuning(param, val)` | Generic handler for all `devTuning` fields — handles `basesloaded_enable` as boolean, all others as int; updates `STORY_ROTATE_MS` + restarts timer when `rotateMs` changes |
...
let devShowPushOnDesktop = false  // when true, 🔔 Game Start Alerts row shows even on desktop; persisted to localStorage('dev_push_desktop')
