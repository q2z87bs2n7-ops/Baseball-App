...
3. **News fallback** — if ESPN API is CORS-blocked, no fallback source.
...
8. **Logo fallback coverage** — `capImgError()` SVG fallback used in home cards only; Around the League matchup cards have no logos.
