...
**Current version:** v2.51 (v1.61 was the final v1 release — v2.x began with the League Pulse merge; [versions 2.2–2.50 omitted for brevity]; v2.51 adds mobile-only article reader modal for News section: creates `/api/proxy-article.js` Vercel function to fetch and parse ESPN articles server-side (bypasses CORS), displays article content in full-screen modal on ≤1024px devices with header, scrollable body, images, and metadata; desktop (≥1025px) keeps current target="_blank" behavior unchanged; modal closes via back button or click-outside
...
| `calcRBICardScore(rbi, event, aScore, hScore, inning, halfInning)` | Returns weighted importance score (0–∞) for a non-HR scoring play. Score ≥ 10 triggers `showRBICard`. Components: base RBI score × hit-type multiplier + context bonus (go-ahead/equalizer/comeback/blowout-suppressor) × inning multiplier. |
