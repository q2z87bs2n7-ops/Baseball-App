...
- [x] ⚡ Pulse — Distinct HR colors: Story Carousel tier-1 HR cards use violet/purple (`rgba(160,100,255)`); feed HR play items use teal/cyan (`rgba(0,195,175)`) via `--hr-bg`/`--hr-border`; previously both shared amber, making carousel and feed look like duplicate content (v2.10)
- [x] ⚡ Pulse — ⚡ Pulse banner: flush-left label + right-extending hairline rule (`var(--border)`) in gap between Story Carousel and feed; bolt uses `var(--accent)`, text in `var(--muted)` uppercase (v2.10)
