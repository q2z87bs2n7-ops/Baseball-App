...
**Current version:** v2.50 (v1.61 was the final v1 release — v2.x began with the League Pulse merge; [versions 2.2–2.49 omitted for brevity]; v2.49.2 lowers key RBI card threshold from ≥30 to ≥10 to surface more scoring-play cards during live games; v2.50 adds mobile-only article reader modal for News section: creates `/api/proxy-article.js` Vercel function to fetch and parse ESPN articles server-side (bypasses CORS), displays article content in full-screen modal on ≤1024px devices with header, scrollable body, images, and metadata; desktop (≥1025px) keeps current target="_blank" behavior unchanged; modal closes via back button or click-outside)
