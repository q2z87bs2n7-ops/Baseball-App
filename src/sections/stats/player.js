...
export async function selectPlayer(id,type,noScroll){
  const playerObj=(state.rosterData[type]||[]).find(function(p){return p.person.id===id;})||{person:{id:id}};
...
function renderPlayerStats(s,group){
  ...
  const html='<div class="player-tab-panels">'+
...
function renderSplitsTab(playerId, group){
  ...
  const opsValues = splits.map(function(s){ return parseFloat(s.stat && s.stat.ops); }).filter(function(v){ return !isNaN(v); });
  ...
  function fmtR(n){ const s = n.toFixed(3); return s.charAt(0)==='0' ? s.slice(1) : s; }
  function row(code){
    const s = byCode[code];
    if(!s || !s.stat) return '';
    const st = s.stat;
  ...
  const html = groupHint + '<div class="splits-grid">'+
...
async function fetchPitchArsenal(playerId){
  ...
  try{
    ...
    const maxPct = arsenal.reduce(function(m,p){ return Math.max(m, p.pct); }, 0);
...
function renderArsenalTab(playerId){
  ...
  const total = arsenal.reduce(function(s,p){ return s + p.pct; }, 0) || 100;
  ...
  const segments = arsenal.map(function(p){
    ...
    const seg = '<circle cx="'+(size/2)+'" cy="'+(size/2)+'" r="'+r+'" fill="none" stroke="'+color+'" stroke-width="'+stroke+'"'+
  ...
  const donut = '<div class="arsenal-donut">'+
...
function renderAdvancedHittingTab(playerId){
  ...
  function pick(){ for(let i=0;i<arguments.length;i++){ const v = num(arguments[i]); if(v != null) return v; } return null; }
  function fmtR(n){ const v = n.toFixed(3); return v.charAt(0)==='0' ? v.slice(1) : v; }
  ...
  const note = '<div class="adv-source-note">Advanced metrics from MLB Stats API · sabermetrics + seasonAdvanced. Statcast (xBA / xwOBA / exit velo / barrel rate) lives on Baseball Savant and is not proxied here.</div>';
...
function avgHeatColor(value){
  ...
  const t = Math.max(0, Math.min(1, (n - 0.150) / (0.380 - 0.150)));
...
function renderHotZoneSection(playerId){
  ...
  function fmtR(v){ const n = parseFloat(v); if(isNaN(n)) return '—'; const s = n.toFixed(3); return s.charAt(0)==='0'?s.slice(1):s; }
...
async function ensureCareerLoaded(playerId, group){
  ...
  try{
    const cR = await fetch(careerUrl).then(function(r){ return r.ok ? r.json() : null; }).catch(function(){ return null; });
...
function renderCareerTab(playerId, group){
  ...
  function fmtR(v){ if(v==null||v==='') return '—'; const n=parseFloat(v); if(isNaN(n)) return String(v); const s=n.toFixed(3); return s.charAt(0)==='0'?s.slice(1):s; }
  function fmtN(v, d){ if(v==null||v==='') return '—'; const n=parseFloat(v); if(isNaN(n)) return String(v); return n.toFixed(d==null?0:d); }
  function fmtIp(v){ if(v==null||v==='') return '—'; return String(v); }
  function intOr(v){ if(v==null||v==='') return '—'; const n=parseInt(v,10); return isNaN(n)?String(v):String(n); }
  ...
  function tableFor(rows, kind){
    ...
    const titleEm = kind === 'hitting' ? '⚾ Hitting' : '🥎 Pitching';
  ...
  const hintHtml = (isMobile && !state.careerSwipeHintShown)
...
export function installStatsQuickNav(){
  ...
  nav.addEventListener('click', function(e){
    ...
    if(!tgt) return;
    const headerH = nav.getBoundingClientRect().bottom; // bottom of quicknav = top of content
  ...
  const ids = Array.prototype.map.call(nav.querySelectorAll('button[data-target]'), function(b){ return b.dataset.target; });
  const targets = ids.map(function(id){ return document.getElementById(id); }).filter(Boolean);
...
function renderGameLogTab(playerId, group){
  ...
  const games = cached.games.slice().reverse().slice(0, 10); // last 10 most recent first
  if(!games.length){
  ...
  if(group==='hitting'){
    ...
    const fmtR = function(n){ const s=n.toFixed(3); return s.charAt(0)==='0'?s.slice(1):s; };
  ...
  games.forEach(function(g){
    const st = g.stat || {};
    const d = g.date ? new Date(g.date+'T12:00:00Z') : null;
...
function computeRollingSeries(games, group, heroKey, windowSize){
  ...
  const ordered = games.slice().reverse(); // oldest → newest
  const out = [];
  if(group==='hitting'){
    const window = [];
...
function renderSparklineSVG(series, opts){
  ...
  const pts = series.map(function(p, idx){ return [pad + idx*step, plotY(p.y)]; });
...
export function switchVsBasis(basis){
  ...
  const entry=pool.find(function(p){return p.player&&p.player.id===sel.person.id;});
