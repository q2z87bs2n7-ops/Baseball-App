...
export function calcRBICardScore(rbi, event, aScore, hScore, inning, halfInning) {
  ...
  const hitMult = event === 'Double' ? 1.5 : event === 'Triple' ? 2.0 :
  ...
  const battingBefore = battingAfter - rbi;
  const deficitBefore = fieldingScore - battingBefore; // positive = batting team was behind
  const marginAfter = battingAfter - fieldingScore;    // positive = batting team now leads
  let ctx = 0;
  ...
  const innMult = inning <= 3 ? 0.4 : inning <= 6 ? 0.75 : inning <= 8 ? 1.0 : inning === 9 ? 1.4 : 1.6;
...
async function resolvePlayerCardData(batterId, batterName, awayTeamId, homeTeamId, halfInning, overrideStats, descHint, gamePk) {
  ...
  const teamData = TEAMS.find(function(t) { return t.id === battingTeamId; }) || {};
  let stat = null, jerseyNumber = null, position = null;
  if (overrideStats) {
    stat = overrideStats;
  } else {
    const cached = (state.statsCache.hitting || []).find(function(e) { return e.player && e.player.id === batterId; });
  ...
  let rEntry = (state.rosterData.hitting || []).find(function(p) { return p.person && p.person.id === batterId; });
  ...
  if ((!position || !jerseyNumber) && gamePk && _fetchBoxscore) {
    try {
      ...
      const playerData = allPlayers.find(function(p) { return p.person && p.person.id === batterId; });
      if (playerData) {
  ...
  if (descHint) { const _m = descHint.match(/\((\d+)\)/); if (_m) { const _n = parseInt(_m[1], 10); if (typeof hrCount !== 'number' || hrCount < _n) hrCount = _n; } }
  return {
    batterId: batterId,
...
export async function showRBICard(batterId, batterName, awayTeamId, homeTeamId, halfInning, rbi, event, aScore, hScore, inning, gamePk) {
  ...
  const battingTeamId = halfInning === 'top' ? awayTeamId : homeTeamId;
  const teamData = TEAMS.find(function(t) { return t.id === battingTeamId; }) || {};
  const awayData = TEAMS.find(function(t) { return t.id === awayTeamId; }) || {};
  const homeData = TEAMS.find(function(t) { return t.id === homeTeamId; }) || {};
...
export function replayHRCard(itemIndex) {
  const hrs = state.feedItems.filter(function(item) { return item.data && item.data.event === 'Home Run'; });
  ...
  const badgeText = play.desc.includes('walk-off') ? 'WALK-OFF HOME RUN!' : '💥 HOME RUN!';
...
export function replayRBICard(itemIndex) {
  const rbis = state.feedItems.filter(function(item) { return item.data && item.data.scoring && item.data.event !== 'Home Run' && item.data.batterId; });
