...
function loadRadioCheckResults() {
  try { const s = localStorage.getItem('mlb_radio_check'); if (s) radioCheckResults = JSON.parse(s) || {}; }
  catch (e) { radioCheckResults = {}; }
  try { const n = localStorage.getItem('mlb_radio_check_notes'); if (n) radioCheckNotes = JSON.parse(n) || {}; }
...
function radioCheckEntries() {
  ...
  Object.keys(MLB_TEAM_RADIO).forEach(function(tid) {
    const team = TEAMS.find(function(t) { return t.id === +tid; });
...
function renderRadioCheckList() {
  ...
  const done = Object.values(radioCheckResults).filter(function(v) { return v === 'yes' || v === 'no'; }).length;
...
export function radioCheckPlay(key) {
  const entries = radioCheckEntries();
  const e = entries.find(function(x) { return x.key === key; });
  if (!e) return;
...
export function radioCheckTryCustom() {
  ...
  const pick = { teamId: null, abbr: 'TEST', name: 'Custom · ' + (fmt === 'hls' ? 'HLS' : 'MP3'), url: url, format: fmt };
  ...
  try {
    ...
    const onPlay = function() { if (status) status.innerHTML = '<span style="color:#22c55e">✅ Playing · ' + fmt.toUpperCase() + ' · ' + escHtml(url.length > 80 ? url.slice(0, 80) + '…' : url) + '</span>'; audio.removeEventListener('playing', onPlay); };
    const onErr = function(e) { if (status) status.innerHTML = '<span style="color:#ff6b6b">❌ Failed · ' + (e && e.message || 'audio error') + '</span>'; audio.removeEventListener('error', onErr); };
...
export function radioCheckCopy() {
  const entries = radioCheckEntries();
  const lines = ['MLB Radio Check Results', 'Date: ' + new Date().toISOString().slice(0, 10), ''];
  ...
  function flash(msg) { if (!btn) return; const orig = btn.textContent; btn.textContent = msg; setTimeout(function() { btn.textContent = orig; }, 1800); }
