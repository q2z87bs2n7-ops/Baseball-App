...
async function loadDailyEventsJSON(){
  try{
    ...
    if(DEBUG) console.log('Demo: Loaded daily-events.json —',Object.keys(data.state.gameStates).length,'games,',data.state.feedItems.length,'plays');
    return data;
  }catch(e){
...
async function initDemo() {
  ...
  if(DEBUG) console.log('Demo: Loaded',state.enabledGames.size,'games from',localDateStr(state.demoDate),',',state.feedItems.length,'feed items,',state.demoPlayQueue.length,'plays queued');
  ...
  updateFeedEmpty();
  showAlert({icon:'▶',event:'Demo Mode',desc:state.enabledGames.size+' games · '+state.feedItems.length+' plays',color:'#7dd89e',duration:3000});
...
async function buildDemoPlayQueue(games) {
  ...
  state.demoPlayQueue.sort(function(a,b){return a.ts-b.ts;});
  state.demoPlayIdx=0;
  if(DEBUG) console.log('Demo: Built queue with',state.demoPlayQueue.length,'plays (real API data only)');
}
...
function exitDemo() {
  ...
  state.pollDateStr=null;  // Will be set by pollLeaguePulse()
  ...
  if(stayingInPulse){
    // Explicitly clear and rebuild feed from empty state.feedItems
    renderFeed();
